...
def calculate_psnr(original_fits, restored_fits):
    ...
    original_data = original_hdul[1].data
    original_hdul.close()
...
# 모든 FITS 파일 처리 및 결과 저장
def process_all_fits(input_folder, output_xlsx, rates):
    results = []
    image_size = 4096 * 4096  # 이미지 크기 (픽셀 수)

    for fits_file in os.listdir(input_folder):
        if fits_file.endswith('.fits'):
            fits_image_filename = os.path.join(input_folder, fits_file)
...
# 메인 함수
def main():
    input_folder = 'data'  # FITS 파일이 있는 폴더 경로
    output_xlsx = 'result13000.xlsx'  # 결과를 저장할 XLSX 파일명
    rates = [13000]  # 여러 가지 압축률

    # 모든 FITS 파일 처리 및 결과 저장
    process_all_fits(input_folder, output_xlsx, rates)
